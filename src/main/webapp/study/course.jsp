			...
			<div class="w pr">
				...
				<div class="statics clearfix"><h1>
					...
					<div class="static-item l">
						...
						<span class="meta-value js-learn-num" id="course_learnCount">1032</span>
					</div>
					<div class="static-item l">
						<span class="meta">难度级别</span> 
						<span class="meta-value" id="course_difficulty">3</span>
						<em></em>
					</div>
					<div class="static-item l">
						<span class="meta">课程时长</span>
						<span class="meta-value" id="course_learnTime">1小时35分</span> <em></em>
					</div>
					<div class="static-item l score-btn">
						<span class="meta">综合评分</span>
						<span class="meta-value" id="course_score">5</span>
						<em></em>
					</div>
					<div class="static-item l">
...
<script type="text/javascript" src="js/ovls.js"></script>
<script type="text/javascript" src="js/ovls_service.js"></script>
<script type="text/javascript" src="js/jquery.SuperSlide.2.1.1.js"></script>
<script type="text/javascript">
$(function(){
	var cid = getParamString("cid");
	if(cid != undefined){
		//加载课程详情
		loadCourseDetail(cid);
		//加载课程章节信息
		loadCourseChapter(cid);
		//加载课程评价信息
		//加载课程笔记信息
	}
});
function loadCourseChapter(cid){
	$("#chapters").show();
	$("#comments").hide();
	$("#notes").hide();
	//加载章节信息
	$.ajax({
		url:VIDEO_SERVICE+"/course/"+cid+"/chapter",
		type:"get",
		dataType:"json",
		success:function(result){
			if(result.code==0){//成功
				var chapters = result.data;//章节集合
				for(var i=0;i<chapters.length;i++){
					var chapter_id = chapters[i].id;//章节id
					var chapter_name = chapters[i].name;//章节名
					var videos = chapters[i].videos;//章节包含的视频集合
					//拼一个章节元素
					var str = '';
					str+='<div class="panel chapter">';
					str+='	<div class="panel-heading">';
					str+='		<h4 class="panel-title">';
					str+='			<a data-toggle="collapse" data-parent="javascript:void(0)"	href="javascript:;">';
					str+='				<h3>';
					str+='					<img alt="" src="${pageContext.request.contextPath}/study/course/icon/lv_icon1.jpg"  id="lv_icon1">';
					str+='					<strong>';
					str+='						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+chapter_name;
					str+='						<img alt="" src="${pageContext.request.contextPath}/study/course/icon/lv_icon2.jpg"  id="lv_icon2">';
					str+='					</strong>';
					str+='					<img alt="" src="${pageContext.request.contextPath}/study/course/icon/lv_icon3.jpg"  id="lv_icon3">';
					str+='				</h3>';
					str+='			</a>';
					str+='		</h4>';
					str+='	</div>';
					...
					for(var j=0;j<videos.length;j++){
						...
						/* var video_url = videos[j].url; */
						str+='		<div class="panel-body">';
						str+='			<a href="video?vid='+video_id+'">';
						str+='				'+video_name;
...
function loadCourseDetail(cid){
	$.ajax({
		url:COURSE_SERVICE+"/course/"+cid,
		type:"get",
		dataType:"json",
		success:function(result){
			if(result.code==0){//成功
				var course = result.data;//课程对象信息
